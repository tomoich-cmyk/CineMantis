...
((SELECT id FROM award_bodies WHERE name = 'Festival de Cannes'), 5, 5, 5, 5, 5, 'https://query.wikidata.org/', 'Q338219', 'Official selection and awards are usually in May.'),
((SELECT id FROM award_bodies WHERE name = 'Venice International Film Festival'), 8, 9, 9, 9, 9, 'https://query.wikidata.org/', 'Q861507', 'Lineup is usually announced in late summer; awards are usually in September.'),
((SELECT id FROM award_bodies WHERE name = 'Berlin International Film Festival'), 2, 2, 2, 2, 2, 'https://query.wikidata.org/', 'Q166332', 'Festival and awards are usually in February.'),
...
((SELECT id FROM award_bodies WHERE name = 'Cesar Awards'), 1, 2, 2, 3, 2, 'https://query.wikidata.org/', 'Q106301', 'Nominations are usually in winter; ceremony/results are usually in February or March.'),
((SELECT id FROM award_bodies WHERE name = 'Goya Awards'), 12, 1, 2, 2, 2, 'https://query.wikidata.org/', 'Q1011547', 'Nominations often span December to January; results are usually in February.'),
((SELECT id FROM award_bodies WHERE name = 'Japan Academy Film Prize'), 1, 1, 3, 3, 3, 'https://query.wikidata.org/', 'Q1033606', 'Main nominations are usually in January; results are usually in March.'),
((SELECT id FROM award_bodies WHERE name = 'Sundance Film Festival'), 1, 1, 1, 1, 1, 'https://query.wikidata.org/', 'Q490839', 'Festival selections and awards are usually in January.'),
((SELECT id FROM award_bodies WHERE name = 'Golden Globe Awards'), 12, 1, 1, 1, 1, 'https://query.wikidata.org/', 'Q1011509', 'Nominations often arrive in December; results are usually in January.'),
((SELECT id FROM award_bodies WHERE name = 'European Film Awards'), 11, 11, 12, 12, 12, 'https://query.wikidata.org/', 'Q1377773', 'Nominations are usually in November; awards are usually in December.')
